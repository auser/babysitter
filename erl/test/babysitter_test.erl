...
test_starting_one_process() ->
  {ok, _ErlProcess, Pid} = babysitter:spawn_run("sleep 2.1", [{env, "HELLO=world"}]),
...
test_exec_one_process() ->
  {ok, Pid} = babysitter:run("sleep 1.7", [{env, "TEST=true"}]),
  CommandArgString = lists:flatten(io_lib:format("ps aux | grep ~p | grep -v grep |  wc -l | tr -d ' '", [Pid])),
  ?assertCmdOutput("0\n", CommandArgString).

test_starting_many_processes() ->
  Count = 200,
  Fun = fun() -> babysitter:spawn_run("sleep 2.2", [{env, "NAME=$NAME"}, {env, "NAME=bob"}]) end,
...
test_killing_a_process() ->
  {ok, _ErlProcess, Pid} = babysitter:spawn_run("sleep 2.6", [{env, "NAME=ari"}]),
...
test_killing_a_process_with_erlang_process() ->
  {ok, ErlProcess, Pid} = babysitter:spawn_run("sleep 201.1", [{env, "NAME=ari"}]),
  ErlProcess ! {stop},
  timer:sleep(2),
  CommandArgString = lists:flatten(io_lib:format("ps aux | grep ~p | grep -v grep | wc -l | tr -d ' '", [Pid])),
  O = ?cmd(CommandArgString),
...
test_listing_processes() ->
  Count = 5,
  Fun = fun() -> babysitter:spawn_run("sleep 2.4", [{env, "NAME=$NAME"}, {env, "NAME=bob"}]) end,
...
test_status_listing_of_a_process() ->
  {ok, _ErlProcess, Pid} = babysitter:spawn_run("sleep 10.8", [{env, "HELLO=world"}]),
